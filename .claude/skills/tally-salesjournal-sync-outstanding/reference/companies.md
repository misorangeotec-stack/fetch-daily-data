...
Both `tools/fetch_tally_journals.py` and `tools/push_journals_to_sheet.py` use this
...
Display values must match the sibling skills
(`tally-sales-sync-outstanding`, `tally-salescreditnote-sync-outstanding`,
`tally-salesdebitnote-sync-outstanding`, `tally-bankreceipt-sync-outstanding`,
`tally-chequereturn-sync-outstanding`, `tally-credit-limit-and-opening-sync-outstanding`)
exactly so that all sheets fed by Tally share the same `(Company, Location)` keys.
Don't drift without updating all skills together — joining or VLOOKUP-ing across
