"""Append fetched journal-voucher rows to the outstanding-report Google Sheet.

Validates the destination tab's row 1 against the schema in
``reference/columns.md``, dedupes by composite
``(Company, Location, Voucher No., Particulars)`` against rows already
present, and appends only the new ones. Particulars is part of the key
because a single journal voucher can post against multiple Sundry-Debtor
parties (one row per debtor leg).
...
Required env vars (loaded from project .env):
    ...
    JOURNAL_SHEET_TAB          tab/sheet name within the spreadsheet (default: Sheet1)

Usage:
    python push_journals_to_sheet.py --input .tmp/journals_<ts>.json
"""
...
DEDUPE_KEYS = ("company", "location", "voucher_no", "particulars", "ref_inv_no")
# A journal's bill allocation can CHANGE in Tally after first sync (booked "On Account", later
# allocated against an invoice → ref_inv_no changes). The stable voucher-party identity below
# (DEDUPE_KEYS minus ref_inv_no) drives the default UPSERT: re-fetching a voucher REPLACES its
# stale allocation rows instead of appending a duplicate.
UPSERT_KEYS = ("company", "location", "voucher_no", "particulars")
DATE_HEADER = "Date"
...
def extract_sheet_id(url: str) -> str:
    ...
    if not m:
        raise SystemExit(f"ERROR: could not extract sheet ID from JOURNAL_SHEET_URL='{url}'")
    return m.group(1)
...
def authorize() -> Any:
    ...
    # (httplib2 has no default timeout — the root cause of the self-heal hang, 2026-06-12).
    import httplib2
...
def append_rows(svc: Any, sheet_id: str, tab: str, rows: list[list[Any]], n_cols: int) -> None:
    if not rows:
        return
    # Pass an explicit column range (A1:<lastcol>1) instead of a single
    # cell. Append's table-boundary auto-detection occasionally 500s on
    # otherwise-empty sheets when given just `A1` — the wider range avoids
    # that. RAW per project convention (CLAUDE.md): USER_ENTERED corrupts
    # phone-number strings and rounds large numbers on round-trip.
...
def _reconcile_sheet(
    ...
    # (DISPLAY names from the data), within the window — NOT by the raw --companies arg
    # (Tally name, never matches the sheet's display name) and NOT by Company alone
    # (would wipe other locations). Empty fetch = no-op. See RECONCILIATION_NOTES.md
    # EC-11 (ported from push_sales_to_sheet.py).
...
def _heal_sheet(
    ...
    NOTE: a single Journal voucher can emit MULTIPLE rows (one per BILLALLOCATIONS entry); they
    all carry the same party ``ledger_id`` and date, so this deletes/re-inserts the whole set
    atomically — correct for the bill-split journals.
    """
...
def _upsert_by_voucher(
    ...
    """Default push = content-aware voucher UPSERT. Groups rows by the voucher identity
    ``upsert_keys`` (``dedupe_keys`` unused, kept for call compatibility) and compares each batch
    voucher's FULL row-set to the sheet's: identical → skip; changed (any column) → delete the
    sheet's rows for that voucher and re-append the fetched rows; new → plain append. So ANY change
    in Tally — a journal re-allocated On-Account→Agst-Ref, an amount correction, a changed
    reference — is reflected on the next covering sync; unchanged vouchers cost nothing; vouchers
    absent from the batch are never touched. Append-first then delete (existing indices stay valid)."""
    ncol = len(columns)
    batch_groups: dict[tuple[str, ...], list[list[Any]]] = {}
    for r in rows_in:
        gk = tuple(str(r.get(k, "")).strip() for k in upsert_keys)
    ...
    if existing and len(existing) > 1:
        ...
        try:
            gk_idx = [headers.index(_header_for(columns, k)) for k in upsert_keys]
        except ValueError as exc:
    ...
    append_rows(svc, sheet_id, tab, to_append, n_cols=ncol)
    if to_delete:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Push fetched journal-voucher JSON to Google Sheets.")
    parser.add_argument("--input", required=True, help="Path to JSON file produced by fetch_tally_journals.py")
    ...
    load_dotenv()
    sheet_url = os.environ["JOURNAL_SHEET_URL"]
    tab = os.environ.get("JOURNAL_SHEET_TAB", "Sheet1")
    ...
    if args.heal:
        ...
        bpath, brows = _backup_tab(svc, sheet_id, tab, backup_dir, f"journal_preheal_{target[-12:]}")
    ...
    stats = _upsert_by_voucher(svc, sheet_id, tab, rows_in, columns, existing, UPSERT_KEYS, DEDUPE_KEYS)
    ...
    summary = {
        ...
        "deleted": stats["deleted"],
        "skipped": len(rows_in) - stats["appended"],
        "sheet_url": sheet_url,
    }
    print(json.dumps(summary))
    return 0
