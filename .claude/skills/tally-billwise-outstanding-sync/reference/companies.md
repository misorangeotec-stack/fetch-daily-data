...
Both `tools/fetch_tally_ledger_master.py` and
`tools/push_ledger_master_to_sheet.py` use this mapping to (a) write clean
...
`tally_name | company | location` (case-insensitive). `tally_name` must match
Tally's company `$Name` EXACTLY (this is what the fetch tool matches on) — it
does NOT include Tally's company number. Any extra columns (e.g. `tally_no`)
are reference-only and ignored by the parser.

## Companies

Display values must match the sibling
`tally-credit-limit-and-opening-sync-outstanding` and
`tally-sales-sync-outstanding` skills exactly so that all sheets fed by
Tally share the same `(Company, Location)` keys. Don't drift without
