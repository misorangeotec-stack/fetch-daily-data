"""Append fetched bank-receipt rows to the outstanding-report Google Sheet.

Validates the destination tab's row 1 against the schema in
``reference/columns.md``, dedupes by composite ``(Company, Location,
Voucher No., Ref Inv No, Allocation Type)`` against rows already present,
...
Required env vars (loaded from project .env):
    ...
    BANK_RECEIPT_SHEET_TAB    tab/sheet name within the spreadsheet

Usage:
    python push_bankreceipt_to_sheet.py --input .tmp/bankreceipt_<ts>.json
"""
...
# A single bank-receipt voucher can split into multiple bill allocations
# (e.g. one receipt settling 3 invoices) AND can credit multiple parties at
# once, so voucher_no alone is not unique. customer_name + bill-ref +
# allocation-type together complete the dedupe key — without customer_name,
# two parties' "On Account" rows on the same voucher would collide.
DEDUPE_KEYS = ("company", "location", "voucher_no", "customer_name", "ref_inv_no", "allocation_type")
# A voucher's bill allocation can CHANGE in Tally after its first sync (e.g. a receipt booked
# "On Account" — blank ref — is later allocated "Agst Ref" against an invoice). The fields below
# are the STABLE voucher-party identity (DEDUPE_KEYS minus ref/alloc). The default push upserts by
# it: re-fetching a voucher REPLACES its stale allocation rows instead of appending a duplicate.
UPSERT_KEYS = ("company", "location", "voucher_no", "customer_name")
DATE_HEADER = "Receipt Date"


def extract_sheet_id(url: str) -> str:
    m = SHEET_ID_RE.search(url)
    if not m:
        raise SystemExit(f"ERROR: could not extract sheet ID from BANK_RECEIPT_SHEET_URL='{url}'")
    return m.group(1)
...
def _reconcile_sheet(
    ...
    # alone would wipe OTHER locations of the same Company. Deriving scope from new_rows
    # also makes an empty fetch a no-op instead of a wipe. See RECONCILIATION_NOTES.md EC-11.
...
def _upsert_by_voucher(
    ...
    """Default push = content-aware voucher UPSERT.

    Groups rows by the voucher identity ``upsert_keys`` (``dedupe_keys`` is unused, kept for call
    compatibility). For each voucher in the fetched batch it compares that voucher's FULL row-set to
    the sheet's:
      * identical            → skip (leave the rows untouched, no rewrite);
      * changed (any column) → delete the sheet's rows for that voucher and re-append the fetched rows;
      * new                  → plain append.

    So ANY change in Tally is reflected on the next covering sync — a receipt re-allocated
    On-Account→Agst-Ref, an amount correction, a changed reference, a removed bill split — while
    unchanged vouchers cost nothing and a forward sync of all-new vouchers is a plain append.
    Vouchers absent from this batch are never touched. Append-first then delete (appended rows land
    at the bottom, so existing row indices stay valid) → no delete-without-reinsert window.
    """
    ncol = len(columns)
    ...
    if to_delete:
        ...
        for gi in sorted(to_delete):                  # merge contiguous indices to minimise requests
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Push fetched bank-receipt JSON to Google Sheets.")
    parser.add_argument("--input", required=True, help="Path to JSON file produced by fetch_tally_bankreceipt.py")
    ...
    load_dotenv()
    sheet_url = os.environ["BANK_RECEIPT_SHEET_URL"]
    tab = os.environ.get("BANK_RECEIPT_SHEET_TAB", "Sheet1")
    ...
    if args.heal:
        ...
        bpath, brows = _backup_tab(svc, sheet_id, tab, backup_dir, f"bankreceipt_preheal_{target[-12:]}")
