...
Both `tools/fetch_tally_bankreceipt.py` and `tools/push_bankreceipt_to_sheet.py` use this
...
Display values must match the sibling `tally-sales-sync-outstanding` and
`tally-credit-limit-and-opening-sync-outstanding` skills exactly so that all
