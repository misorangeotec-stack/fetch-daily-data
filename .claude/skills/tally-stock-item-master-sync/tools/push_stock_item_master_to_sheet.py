"""Upsert fetched stock-item-master rows into the Tally Stock Item Master Google Sheet.

For each fetched row, the upsert key is the composite ``(Company, Location,
item_id)``. If a sheet row already has that key, the script **updates the
...
Required env vars (loaded from project .env):
    ...
    STOCK_ITEM_MASTER_SHEET_TAB      tab/sheet name within the spreadsheet

Usage:
    python push_stock_item_master_to_sheet.py --input .tmp/stock_item_master_<ts>.json
"""
...
UPSERT_KEYS = ("company", "location", "item_id")  # composite key for upsert
...
def extract_sheet_id(url: str) -> str:
    ...
    if not m:
        raise SystemExit(f"ERROR: could not extract sheet ID from STOCK_ITEM_MASTER_SHEET_URL='{url}'")
    return m.group(1)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upsert Tally stock-item-master JSON into Google Sheets.")
    parser.add_argument("--input", required=True, help="Path to JSON file produced by fetch_tally_stock_item_master.py")
    args = parser.parse_args()

    load_dotenv()
    sheet_url = os.environ["STOCK_ITEM_MASTER_SHEET_URL"]
    tab = os.environ.get("STOCK_ITEM_MASTER_SHEET_TAB", "Sheet1")
