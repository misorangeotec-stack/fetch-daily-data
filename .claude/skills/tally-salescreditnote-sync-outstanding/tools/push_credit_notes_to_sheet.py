"""Append fetched credit-note rows to the outstanding-report Google Sheet.
...
Required env vars (loaded from project .env):
    ...
    CREDIT_NOTE_SHEET_TAB      tab/sheet name within the spreadsheet (default: Sheet1)

Usage:
    python push_credit_notes_to_sheet.py --input .tmp/credit_notes_<ts>.json
"""
...
# The default push UPSERTS by this voucher identity (content-aware): re-fetching a voucher whose
# data changed in Tally (amount correction, changed reference, etc.) REPLACES its row instead of
# leaving the stale one; unchanged vouchers are skipped. See _upsert_by_voucher.
...
def extract_sheet_id(url: str) -> str:
    ...
    if not m:
        raise SystemExit(f"ERROR: could not extract sheet ID from CREDIT_NOTE_SHEET_URL='{url}'")
    return m.group(1)
...
def _reconcile_sheet(
    ...
    # The sheet stores Company + Location separately and as DISPLAY names, so filtering
    # by the raw --companies arg (or by Company alone) is wrong: it either matches nothing
    # (raw vs display) or wipes OTHER locations of the same Company. Deriving scope from
    # new_rows also makes an empty fetch a no-op instead of a destructive wipe.
...
def _upsert_by_voucher(
    ...
    ``upsert_keys``; for each batch voucher it compares the FULL row-set to the sheet's: identical →
    skip; changed (any column) → delete the sheet's rows for that voucher + re-append; new → plain
    append. So an amount correction / changed reference in Tally is reflected on the next covering
    sync; unchanged vouchers cost nothing; vouchers absent from the batch are never touched.
    Append-first then delete (existing indices stay valid)."""
    ncol = len(columns)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Push fetched credit-note JSON to Google Sheets.")
    parser.add_argument("--input", required=True, help="Path to JSON file produced by fetch_tally_credit_notes.py")
    ...
    load_dotenv()
    sheet_url = os.environ["CREDIT_NOTE_SHEET_URL"]
    tab = os.environ.get("CREDIT_NOTE_SHEET_TAB", "Sheet1")
    ...
    if args.heal:
        ...
        bpath, brows = _backup_tab(svc, sheet_id, tab, backup_dir, f"creditnote_preheal_{target[-12:]}")
    ...
    stats = _upsert_by_voucher(svc, sheet_id, tab, rows_in, columns, existing, UPSERT_KEYS)
