# Output Schema — Outstanding Sales Credit Note Sheet

This file is the **single source of truth** for the columns written to the destination Google Sheet by the `tally-salescreditnote-sync-outstanding` skill. Both `tools/fetch_tally_credit_notes.py` and `tools/push_credit_notes_to_sheet.py` parse this file at runtime via `tools/_schema.py`.
...
The `key` column is the JSON key used inside the intermediate `.tmp/credit_notes_*.json` file (lowercase, snake_case-ish, no special chars). It must be unique.
...
| type            | Type                         | Constant `credit note` (matches reference Excel)              |
...
| against_invoice | Against Sales Invoice no.    | Voucher-level REFERENCE field (the original sales invoice)    |
