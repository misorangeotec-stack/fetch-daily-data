# Output Schema — Tally Profit & Loss Sheet

This file is the **single source of truth** for the columns written to the
destination Google Sheet by the `tally-pnl-sync` skill. Both
`tools/fetch_tally_pnl.py` and `tools/push_pnl_to_sheet.py` parse this file at
runtime via `tools/_schema.py`.
...
`.tmp/pnl_*.json` file (lowercase, snake_case). It must be unique.
...
| statement             | statement             | Always `Profit & Loss` for this skill (derived; see classification)     |
| side                  | side                  | Derived: Income / Expense                                              |
| closing_balance       | closing_balance       | Group net for FY-start → as_of_date from Tally's Trial Balance, sign-flipped (positive = Dr/Expense, negative = Cr/Income) |
...
`tools/push_pnl_to_sheet.py` as well.
...
- **`opening_balance`** is the **absolute value**; the sign is captured
  ...
  if >0, blank if zero). (For most P&L groups the opening is zero at
  FY-start.)
- **`closing_balance`** is **signed and flipped** — positive = Dr,
  negative = Cr, zero = zero. So Expense groups (debit) show positive and
  Income groups (credit) show negative. It represents the **period**
  (FY-start → as_of_date) income/expense, since SVFROMDATE is the FY-start.
...
First match wins; edit `STATEMENT_RULES` in `tools/fetch_tally_pnl.py` to
adjust.
...
**This skill keeps only `statement == "Profit & Loss"` rows.** Asset /
Liability groups are counted in `excluded_other_statement` and dropped (they
belong to the sibling `tally-balance-sheet-sync` skill).

**Nett Profit cross-check:** sum of `Income`-side `closing_balance` (these
are negative = Cr) minus sum of `Expense`-side (positive = Dr) — i.e.
`-(Income total) - (Expense total)` — should match Tally's reported Nett
Profit for the company at the as-of date.
