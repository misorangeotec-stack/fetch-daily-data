...
Both `tools/fetch_tally_credit_notes.py` and `tools/push_credit_notes_to_sheet.py` use this
...
(`tally-sales-sync-outstanding`, `tally-credit-limit-and-opening-sync-outstanding`,
`tally-bankreceipt-sync-outstanding`) exactly so that all sheets fed by Tally
share the same `(Company, Location)` keys. Don't drift without updating all
skills together — joining or VLOOKUP-ing across sheets depends on this consistency.
