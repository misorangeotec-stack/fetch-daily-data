"""Upsert fetched ledger-master rows into the Tally Ledger Master Google Sheet.

For each fetched row, the upsert key is the composite ``(Company, Location,
ledger_id)``. If a sheet row already has that key, the script **updates the
schema-managed columns in place** (preserving any extra columns the user
has added to the right). If the key is new, the row is **appended** at the
bottom.

Timestamp handling:
  * ``created_at`` and ``updated_at`` are populated by this push tool, not
    by the fetch tool (the fetch JSON leaves them blank).
...
Required env vars (loaded from project .env):
    ...
    LEDGER_MASTER_SHEET_TAB      tab/sheet name within the spreadsheet

Usage:
    python push_ledger_master_to_sheet.py --input .tmp/ledger_master_<ts>.json
"""
...
UPSERT_KEYS = ("company", "location", "ledger_id")  # composite key for upsert
...
def now_iso() -> str:
    ...
    Using one fixed value across the whole run means that all rows
    inserted/updated in a single push share the same timestamp, which is
    nicer for downstream filtering than per-row drift.
    """
...
def extract_sheet_id(url: str) -> str:
    ...
    if not m:
        raise SystemExit(f"ERROR: could not extract sheet ID from LEDGER_MASTER_SHEET_URL='{url}'")
    return m.group(1)
...
def existing_key_to_row(existing: list[list[str]], columns: list[Column]) -> dict[tuple[str, ...], int]:
    ...
    Skips data rows whose key is entirely blank. The first data row is at
    row 2 (row 1 = header).
    """
    if len(existing) <= 1:
        return {}
...
def _normalize(v: Any) -> str:
    ...
    numbers, so "800000.00" round-trips back to us as "800000". Try a float
    parse first; fall back to a stripped string.
    """
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Upsert Tally ledger-master JSON into Google Sheets.")
    parser.add_argument("--input", required=True, help="Path to JSON file produced by fetch_tally_ledger_master.py")
    args = parser.parse_args()

    load_dotenv()
    sheet_url = os.environ["LEDGER_MASTER_SHEET_URL"]
    tab = os.environ.get("LEDGER_MASTER_SHEET_TAB", "Sheet1")
    ...
    # Indices of the timestamp columns in `columns` order (for excluding from
    # equality comparison and for surgical preservation of created_at).
    try:
        created_at_idx = _key_index_in_columns(columns, "created_at")
        updated_at_idx = _key_index_in_columns(columns, "updated_at")
    except ValueError:
        # Schema doesn't have timestamps — fall back to "no special handling"
        created_at_idx = -1
        updated_at_idx = -1
    ...
    for r in rows_in:
        ...
        # Skip rows missing any part of the upsert key (would collide on insert).
        if not all(key):
            continue
        ...
        if key in key_to_sheet_row:
            ...
            # Compare only the non-timestamp columns. If those match, the row
            # is genuinely unchanged and we leave the timestamps alone.
            existing_compare = [
            ...
            # Data changed → preserve the existing created_at, bump updated_at.
            if created_at_idx >= 0:
            ...
            # Insert → both timestamps get the run timestamp.
            if created_at_idx >= 0:
