"""Fetch the Balance Sheet (group level) from Tally Prime via XML over HTTP.
...
For ONE user-chosen company:
  ...
  3. Keep the rows that are groups (full group tree, classified into
     statement / side), filter to **Balance Sheet** groups, and write them.
...
Period model (single `--as-of DD-MM-YYYY`, default = today):
  * SVTODATE   = --as-of  (closing as of this date).
  * SVFROMDATE = 1-Apr of the FY containing --as-of (Indian FY).

Sign convention (mirrors tally-ledger-master-sync):
  * `closing_balance` is sign-flipped — positive = Dr, negative = Cr. So
    Assets (debit) show positive and Liabilities (credit) show negative.

Required env vars (loaded from project .env):
    TALLY_HOST   e.g. http://localhost:9000

Usage:
    python fetch_tally_balance_sheet.py --company "Co Name" \\
        [--as-of 15-06-2026] --output .tmp/balance_sheet_<ts>.json
"""
...
# This skill keeps only rows belonging to this statement (the P&L skill is
# identical except STATEMENT = "Profit & Loss").
