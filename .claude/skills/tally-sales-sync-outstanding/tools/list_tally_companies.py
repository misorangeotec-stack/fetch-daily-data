...
def list_companies(host: str) -> list[str]:
    try:
        resp = requests.post(host, data=REQUEST_XML.encode("utf-8"), timeout=180)
        ...
        raise SystemExit(
            f"ERROR: Tally at {host} accepted the connection but did not respond within 180 s. "
            "Tally may be busy — wait a moment and retry. "
            f"({exc})"
        )
