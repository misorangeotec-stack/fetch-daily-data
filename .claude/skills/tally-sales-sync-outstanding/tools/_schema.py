"""Shared schema/config loaders for the tally-sales-sync-outstanding skill.
...
class Column:
    key: str          # JSON key used in .tmp/sales_*.json
...
DETAILS_SCHEMA_FILE = REFERENCE_DIR / "columns_details.md"
...
def load_details_columns() -> List[Column]:
    """Schema for the bill-wise detail sheet (one row per BILLALLOCATIONS entry).

    Parsed from ``reference/columns_details.md``. Distinct from ``load_columns()``
    which governs the per-voucher summary sheet.
    """
    return load_columns(DETAILS_SCHEMA_FILE)
