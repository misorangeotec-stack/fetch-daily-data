...
Both `tools/fetch_tally_sales.py` and `tools/push_sales_to_sheet.py` use this
...
`tally-credit-limit-and-opening-sync-outstanding` skill exactly so that all
sheets fed by Tally share the same `(Company, Location)` keys. Don't drift
without updating both skills together — joining or VLOOKUP-ing across
